...
var _Sources = (() => {
  ...
  var TruyenQQ_exports = {};
  __export(TruyenQQ_exports, {
    TruyenQQ: () => TruyenQQ,
    TruyenQQInfo: () => TruyenQQInfo
  });
  var import_types2 = __toESM(require_lib());

  // src/TruyenQQ/TruyenQQParser.ts
  ...
  var isLastPage = ($2) => {
    let isLast = false;
    const pages = [];
    for (const page of $2("li", "ul.pagination-list").toArray()) {
      const p = Number($2("a", page).text().trim());
      if (isNaN(p)) continue;
      pages.push(p);
    }
    const lastPage = Math.max(...pages);
    const currentPage = Number($2("li > a.is-current").text().trim());
  ...
  var TruyenQQ = class {
    ...
    async getMangaDetails(mangaId) {
      ...
      let creator = [];
      let status = "";
      let desc = $2(".story-detail-info").text();
      for (const t of $2("a", ".list01").toArray()) {
        const genre = $2(t).text().trim();
    ...
    async getSearchTags() {
      ...
      const arrayTags4 = [];
      const arrayTags5 = [];
      for (const tag of $2("div.genre-item", "div.col-sm-10").toArray()) {
        const label = $2(tag).text().trim();
        const id = $2("span", tag).attr("data-id") ?? label;
        if (!id || !label) continue;
        arrayTags.push({ id, label });
      }
      for (const tag of $2("option", "select#country").toArray()) {
        const label = $2(tag).text().trim();
        const id = "country." + $2(tag).attr("value");
        if (!id || !label) continue;
        arrayTags2.push({ id, label });
      }
      for (const tag of $2("option", "select#status").toArray()) {
        const label = $2(tag).text().trim();
        const id = "status." + $2(tag).attr("value");
        if (!id || !label) continue;
        arrayTags3.push({ id, label });
      }
      for (const tag of $2("option", "select#minchapter").toArray()) {
        const label = $2(tag).text().trim();
        const id = "minchapter." + $2(tag).attr("value");
        if (!id || !label) continue;
        arrayTags4.push({ id, label });
      }
      for (const tag of $2("option", "select#sort").toArray()) {
        const label = $2(tag).text().trim();
        const id = "sort." + $2(tag).attr("value");
        if (!id || !label) continue;
        arrayTags5.push({ id, label });
      }
      const tagSections = [
  ...
  return __toCommonJS(TruyenQQ_exports);
})();
